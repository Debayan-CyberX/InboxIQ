import { useState, useEffect } from "react";
import { X, Sparkles, Send, Edit3, RotateCcw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

interface EmailPreviewPanelProps {
  isOpen: boolean;
  onClose: () => void;
  onSend?: () => void | Promise<void>;
  email: {
    to: string;
    subject: string;
    draft: string;
    reason: string;
    company: string;
  } | null;
}

const tones = [
  { id: "professional", label: "Professional" },
  { id: "short", label: "Short" },
  { id: "confident", label: "Confident" },
  { id: "polite", label: "Polite" },
  { id: "sales", label: "Sales-focused" },
];

const EmailPreviewPanel = ({ isOpen, onClose, onSend, email }: EmailPreviewPanelProps) => {
  const [selectedTone, setSelectedTone] = useState("professional");
  const [isEditing, setIsEditing] = useState(false);
  const [draftContent, setDraftContent] = useState(email?.draft || "");
  const [isSending, setIsSending] = useState(false);

  // Update draft content when email changes
  useEffect(() => {
    if (email?.draft) {
      setDraftContent(email.draft);
    }
  }, [email?.draft]);

  const handleSend = async () => {
    if (!onSend) return;
    
    try {
      setIsSending(true);
      await onSend();
    } catch (error) {
      console.error("Error in send handler:", error);
    } finally {
      setIsSending(false);
    }
  };

  if (!isOpen || !email) return null;

  return (
    <div className="fixed inset-y-0 right-0 w-[480px] bg-card border-l border-border shadow-xl z-50 animate-slide-in flex flex-col">
      {/* Header */}
      <div className="p-4 border-b border-border flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-accent" />
          <h3 className="font-semibold text-foreground">AI-Generated Draft</h3>
        </div>
        <Button variant="ghost" size="icon-sm" onClick={onClose}>
          <X className="w-4 h-4" />
        </Button>
      </div>

      {/* Content */}
      <div className="flex-1 overflow-y-auto">
        {/* AI Reason */}
        <div className="p-4 bg-accent/5 border-b border-border">
          <p className="text-sm text-foreground/80 break-words">
            <span className="font-medium text-accent">Why this email:</span>{" "}
            {email.reason}
          </p>
        </div>

        {/* Email form */}
        <div className="p-4 space-y-4">
          {/* To */}
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">To</label>
            <p className="mt-1 text-sm text-foreground font-medium break-all">{email.to}</p>
          </div>

          {/* Subject */}
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Subject</label>
            <input
              type="text"
              value={email.subject}
              className="mt-1 w-full h-9 px-3 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring truncate"
              readOnly={!isEditing}
            />
          </div>

          {/* Tone selector */}
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Tone</label>
            <div className="mt-2 flex flex-wrap gap-2">
              {tones.map((tone) => (
                <button
                  key={tone.id}
                  onClick={() => setSelectedTone(tone.id)}
                  className={cn(
                    "px-3 py-1.5 rounded-full text-xs font-medium transition-all",
                    selectedTone === tone.id
                      ? "bg-accent text-accent-foreground"
                      : "bg-muted text-muted-foreground hover:bg-muted/80"
                  )}
                >
                  {tone.label}
                </button>
              ))}
            </div>
          </div>

          {/* Draft content */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Message</label>
              <Button
                variant="ghost"
                size="sm"
                className="h-7 text-xs gap-1"
                onClick={() => setIsEditing(!isEditing)}
              >
                <Edit3 className="w-3 h-3" />
                {isEditing ? "Done" : "Edit"}
              </Button>
            </div>
            <textarea
              value={draftContent || email.draft}
              onChange={(e) => setDraftContent(e.target.value)}
              readOnly={!isEditing}
              className={cn(
                "w-full h-64 p-4 rounded-lg border text-sm leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-ring",
                isEditing ? "bg-background border-border" : "bg-muted/30 border-transparent"
              )}
            />
          </div>
        </div>
      </div>

      {/* Actions */}
      <div className="p-4 border-t border-border bg-muted/30 shrink-0">
        <div className="flex items-center gap-3">
          <Button 
            variant="outline" 
            className="gap-2" 
            onClick={() => setDraftContent(email.draft)}
            disabled={isSending}
          >
            <RotateCcw className="w-4 h-4" />
            Regenerate
          </Button>
          <Button 
            variant="accent" 
            className="flex-1 gap-2"
            onClick={handleSend}
            disabled={isSending || !onSend}
          >
            <Send className="w-4 h-4" />
            {isSending ? "Sending..." : "Send Email"}
          </Button>
        </div>
        <p className="text-xs text-muted-foreground text-center mt-3">
          You're always in control. Nothing sends without your approval.
        </p>
      </div>
    </div>
  );
};

export default EmailPreviewPanel;
